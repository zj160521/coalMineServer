...
@Scope("prototype")
@Controller
@RequestMapping("/switchStateChange")
public class SwitchStateChangeController {
	...
	@Autowired
	private SwitchStateChangeService service;
	...
	public Object getall(@RequestBody NameTime nameTime,HttpServletRequest request){
		...
		try {
			...
			if(start==true){
				List<SwitchStateChange> list = service.getall(nameTime);
				if(list.size()==0){
